...
s str disamb(c c_, i lvl_) {
   drawdisamb(c_, lvl_, 0); c inp_; inp_=getchar();
   if (inp_<'a' || inp_>'z') return 0; else return find(c_, inp_-'a'); }
s str hunt() {
   c c_; c_=getchar();
   if (count(c_)==1) return find(c_, 0); else return disamb(c_, 1); }
s v cmdloop() {
